'use client'
// Student Groups — real-time chat via SocketIO (brighter-nepal-chat service)
import { useEffect, useState, useRef } from 'react'
import { BadgeCheck, MoreVertical, Search, Smile, Image, Send, Loader2, Wifi, WifiOff, Users } from 'lucide-react'
...
export default function GroupsPage() {
  ...
  const { messages, sendMessage, setTyping, typingUsers, onlineCount, connected } = useGroupChat(group?.id ?? null)
    ...
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-surface-container-low">
      <section className="flex-1 flex flex-col bg-white overflow-hidden">
        ...
        <header className="p-5 bg-white border-b border-surface-container flex justify-between items-center flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            {/* Connection status */}
            <span title={connected ? 'Live — real-time' : 'Disconnected'}>
              {connected
              ...
              <div key={msg.id}>
                ...
                <div className={cn("flex flex-col max-w-2xl", isMe ? "items-end ml-auto" : "items-start")}>
                  ...
                  <div className={cn("p-4 rounded-2xl shadow-sm max-w-xl",
                    isMe
                      ? "bg-[#c0622f] text-white rounded-tr-none"
                      : "bg-white border border-slate-100 text-[#1a1a4e] rounded-tl-none",
